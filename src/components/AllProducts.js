...
const AllProducts = () => {
  ...
  const [maxPrice, setMaxPrice] = useState(10000); // Default max range
  const [priceRange, setPriceRange] = useState([minPrice, maxPrice]);

  // Get the minimum and maximum price from the products
  const productPrices = allProducts.map(product => parseFloat(product.price || product.variants?.[0]?.price || 0));
  ...
  useEffect(() => {
    const filtered = allProducts.filter((product) => {
      const productPrice = parseFloat(product.price || product.variants?.[0]?.price || 0);
  ...
  return (
    <div className="container mx-auto py-12 px-6 md:px-16 lg:px-18 pt-28 bg-gradient-to-b from-pink-50 via-white to-pink-100">
      <h1 className="text-4xl font-extrabold text-center text-pink-600 mb-12">All Sweets</h1>
      ...
      <div className="flex justify-center items-center space-x-6 mb-10">
        <div className="flex flex-col items-center">
          <label className="font-semibold text-lg text-pink-600">Min Price: ₹{priceRange[0]}</label>
          <input
            type="range"
            min={minProductPrice}
            max={maxProductPrice}
        ...
        <div className="flex flex-col items-center">
          <label className="font-semibold text-lg text-pink-600">Max Price: ₹{priceRange[1]}</label>
          <input
            type="range"
            min={minProductPrice}
            max={maxProductPrice}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {filteredProducts.length > 0 ? (
          filteredProducts.map((product) => (
            <div key={product.id} className="border rounded-lg overflow-hidden shadow-lg transition duration-300 transform hover:scale-105 hover:shadow-2xl bg-white">
              <img
              ...
              />
              <div className="p-6">
                <h2 className="font-bold text-xl text-gray-800 truncate">{product.title}</h2>
                <p className="text-gray-600 text-sm mt-2 h-16 overflow-hidden">
                  {product.description ? (
                    <DecodeDescription description={product.description} />
                  ) : (
                    product.body_html && <DecodeDescription description={product.body_html} />
                  )}
                </p>
